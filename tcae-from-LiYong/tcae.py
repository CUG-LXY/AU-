...
def criterion_tv(mat):
    res = torch.abs(mat).contiguous().view(mat.size(0), -1).mean(dim=1)
    return res

def criterion_tv_val(mat):
    res = torch.abs(mat).contiguous().view(mat.size(0), -1).mean()
    return res
...
def train(epoch, model, criterion, preceptual_criterion, optimizer, w1=1.0, w2=1.0, w3=1.0, w4=1.0, \
    ...
    for iteration, batch in enumerate(training_data_loader, 1):
        optimizer.zero_grad()
        # suppose 
        target_image = Variable(batch[0])
        offset = 1
        input_images = batch[offset:]
        input_images = Variable(input_images[0])
        ...
        pose_grid = torch.Tensor(pose_grid).unsqueeze(0).repeat(target_pose_f.size(0), 1,1,1)
        ...
        if epoch >= epoch_t:
            ...
            pose_2grid = torch.Tensor(pose_2grid).unsqueeze(0).repeat(pose_img_pose_f.size(0), 1,1,1)
        ...
        exp_grid = torch.Tensor(exp_grid).unsqueeze(0).repeat(target_exp_f.size(0), 1,1,1)
        ...
        if epoch >= epoch_t:
            ...
            exp_2grid = torch.Tensor(exp_2grid).unsqueeze(0).repeat(exp_img_exp_f.size(0), 1,1,1)
        ...
        merge_grid = torch.Tensor(merge_grid).unsqueeze(0).repeat(exp_vector.size(0), 1,1,1)
...
def val(epoch, model, criterion, criterion_feature_recon, optimizer,  w1=1.0, w2=1.0, w3=0.1, \
    ...
    val_data_loader = DataLoader(dataset=val_set, 
            num_workers=opt.num_workers, batch_size=int(opt.batch_size/2), shuffle=False)
